...
[Client Hints](https://github.com/igrigorik/http-client-hints) move the complexities of alternate resources, `x` descriptors, and `w` descriptors out of `srcset` attributes and into HTTP headers. Using Client Hints, markup can point to a single `src`. Browsers send along their `device-pixel-ratio` and (optionally) the layout width of the image as headers in the request for that `src`; the server can then choose to send an alternate, more appropriate resource in response.
